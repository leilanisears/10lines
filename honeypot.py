...
def msg_received(msg, ip_address):
    sender_mail = "leilani.sears@gmail.com"
    receiver_mail = "leilani.sears@gmail.com"
    ...
    context = ssl.create_default_context()
    message = """\
        Sender %s has sent the following message:

        %s

        Message stored in logs.""" % (ip_address, msg)

    with smtplib.SMTP_SSL(smtp_server, port, context=context) as server:
        server.login(sender_email, password)
        server.send_mail(sender_email, receiver_email, message)
